...
# The EDAutopilot class from ED_AP.py
...
class EDAutopilot:
    ...
    def __init__(self, cb, use_gpu_ocr=False, doThread=True):
        ...
        if cnf is not None:
            ...
            self.config = cnf
            logging.debug("read AP json:"+str(cnf))
        else:
            self.write_config(self.config)
        ...
        if shp_cnf is not None:
            if len(shp_cnf) != len(self.ship_configs):
                ...
                logging.debug("read Ships Config json:" + str(shp_cnf))
            else:
                self.ship_configs = shp_cnf
                logging.debug("read Ships Config json:" + str(shp_cnf))
        else:
    ...
    def read_config(self, fileName='./configs/AP.json'):
        ...
        try:
            ...
            logging.warning("EDAPGui.py read_config error :"+str(e))
    ...
    def write_config(self, data, fileName='./configs/AP.json'):
        try:
            ...
            logging.warning("EDAPGui.py write_config error:"+str(e))
    ...
    def read_ship_configs(self, filename='./configs/ship_configs.json'):
        ...
        try:
            ...
            logging.warning("EDAPGui.py read_ship_configs error :"+str(e))
    ...
    def update_ship_configs(self):
        ...
        if self.current_ship_type in ship_size_map:
            ...
            if self.current_ship_type not in self.ship_configs['Ship_Configs']:
                ...
                logging.debug(f"Created new ship config entry for: {self.current_ship_type}")
            ...
            self.write_ship_configs(self.ship_configs)
            logging.debug(f"Saved ship config for: {self.current_ship_type}")
    ...
    def write_ship_configs(self, data, filename='./configs/ship_configs.json'):
        ...
        try:
            ...
            logging.warning("EDAPGui.py write_ship_configs error:"+str(e))
    ...
    def load_ship_configuration(self, ship_type):
        ...
        if ship_type in self.ship_configs['Ship_Configs']:
            ...
            if any(key in current_ship_cfg for key in ['compass_scale', 'RollRate', 'PitchRate', 'YawRate', 'SunPitchUp+Time', 'AutoDockBoost', 'AutoDockForwardTime', 'AutoDockDelayTime']):
                ...
                logging.info(f"Loaded your custom configuration for {ship_type} from ship_configs.json")
                return
        ...
        if ship_type in ship_rpy_sc_50:
            ...
            logging.info(f"Loaded default configuration for {ship_type} from default ship cfg file")
            return
        ...
        self.autodock_forward_time = 2
        self.autodock_delay_time = 12
        logging.info(f"Using hardcoded default configuration for {ship_type}")
    ...
    def get_cargo_info(self):
        ...
        if capacity is None:
            logging.warning("Cargo capacity not available from journal.")
            # Fallback or default value if necessary
            capacity = 0

        if current is None:
            logging.warning("Current cargo not available from status.")
            # Fallback or default value if necessary
            current = 0
    ...
    def fss_detect_elw(self, scr_reg):
        ...
        logging.info("elw detected:{0:6.2f} ".format(maxVal)+" sig:{0:6.2f}".format(maxVal1))
        ...
        if maxVal1 > 0.70 and maxLoc1[1] < 30:
            ...
            logging.info(sstr+" world at: "+str(self.jn.ship_state()["location"]))
        else:
            self.fss_detected = "nothing found"
    ...
    def have_destination(self, scr_reg) -> bool:
        ...
        logging.debug("has_destination:"+str(maxVal))
    ...
    def is_destination_occluded(self, scr_reg) -> bool:
        ...
        if maxVal > scr_reg.target_occluded_thresh:
            logging.debug(f"Target is occluded ({maxVal:5.4f} > {scr_reg.target_occluded_thresh:5.2f})")
            return True
        else:
            #logging.debug(f"Target is not occluded ({maxVal:5.4f} < {scr_reg.target_occluded_thresh:5.2f})")
            return False
    ...
    def sc_disengage_label_up(self, scr_reg) -> bool:
        ...
        if self.cv_view:
            ...
            cv2.imshow('sc_disengage_label_up', self.cv_view_x-460,self.cv_view_y+575)
            cv2.waitKey(1)
    ...
    def sc_disengage(self, scr_reg) -> bool:
        ...
        if self.cv_view:
            ...
            dis_image = cv2.rectangle(dis_image, (0, 0), (1000, 25), (0, 0, 0), -1)
            cv2.putText(dis_image, f'{maxVal:5.4f} > {scr_reg.disengage_thresh}', (1, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.imshow('disengage', self.cv_view_x-460,self.cv_view_y+575)
            cv2.waitKey(1)

        #logging.debug("Disenage = "+str(maxVal))

        if maxVal > scr_reg.disengage_thresh:
            logging.info("'PRESS [] TO DISENGAGE' detected. Disengaging Supercruise")
    ...
    def sc_disengage_active(self, scr_reg) -> bool:
        ...
        if ocr_textlist is not None:
            ...
            logging.info(f"Disengage similarity with {str(ocr_textlist)} is {sim}")
        ...
        if self.cv_view:
            ...
            cv2.imshow('disengage2', self.cv_view_x - 460, self.cv_view_y + 650)
            cv2.waitKey(30)
    ...
    def dock(self):
        ...
        while self.jn.ship_state()['status'] != 'in_space':
            ...
            if ((time.time() - starttime) > 30):
                self.keys.send('SetSpeedZero')
                logging.error('In dock(), after long wait, but still not in_space')
        ...
        if not granted:
            ...
            logging.warning('Did not get docking authorization, reason:'+str(self.jn.ship_state()['no_dock_reason']))
            ...
            logging.warning('Auto dock timer timed out. Aborting Docking.')
    ...
    def sun_avoid(self, scr_reg):
        ...
        while self.is_sun_dead_ahead(scr_reg):
            ...
            if ((time.time()-starttime) > fail_safe_timeout):
                logging.debug('sun avoid failsafe timeout')
    ...
    def nav_align(self, scr_reg):
        ...
        if not (self.jn.ship_state()['status'] == 'in_supercruise' or self.jn.ship_state()['status'] == 'in_space'):
            logging.error('align=err1, nav_align not in super or space')
        ...
        for ii in range(self.config['NavAlignTries']):
            ...
            sleep(.1)
            logging.debug("final x:"+str(off['x'])+" y:"+str(off['y']))
    ...
    def fsd_target_align(self, scr_reg):
        ...
        logging.debug('align= fsd_target_align')
        ...
        while True:
            ...
            if not self.have_destination(scr_reg):
                logging.debug("Compass not found, cannot align.")
                sleep(1)
                continue
    ...
    def mnvr_to_target(self, scr_reg):
        logging.debug('align')
        if not (self.jn.ship_state()['status'] == 'in_supercruise' or self.jn.ship_state()['status'] == 'in_space'):
            logging.error('align() not in sc or space')
    ...
    def sc_align(self, scr_reg) -> bool:
        ...
        while True:
            ...
            if self.jn.ship_state()['status'] != 'in_supercruise':
                ...
                #     logging.debug("Gliding")
                #     self.status.wait_for_flag2_off(Flags2GlideMode, 30)
                #     return True  # Success
                # else:
                logging.debug("No longer in supercruise")
            ...
            if use_target_align:
                ...
                logging.debug(f"Compass alignment attempt: {compass_align_count}")
    ...
    def position(self, scr_reg, did_refuel=True):
        ...
        if not self.smart_sleep_fsd(pause_time, scr_reg):
            logging.warning("Positioning maneuver interrupted.")
    ...
    def jump(self, scr_reg):
        ...
        for i in range(jump_tries):
            ...
            if not (self.jn.ship_state()['status'] == 'in_supercruise' or self.jn.ship_state()['status'] == 'in_space'):
                logging.error('Not ready to FSD jump. jump=err1')
            ...
            if not res:
                logging.error('FSD failed to charge.')
                continue

            res = self.status.wait_for_flag_on(FlagsFsdJump, 30)
            if not res:
                logging.warning('FSD failure to start jump timeout.')
            ...
            if not res:
                logging.error('FSD failure to complete jump timeout.')
                continue
        ...
        logging.error(f'FSD Jump failed {jump_tries} times. jump=err2')
    ...
    def refuel(self, scr_reg):
        ...
        if self.jn.ship_state()['fuel_percent'] < self.config['RefuelThreshold'] and is_star_scoopable and has_fuel_scoop:
            ...
            logging.debug('No fuel scoop fitted.')
            self.pitchUp(20)
            return False
        ...
        if not on_planet:
            ...
            if self.status.get_flag(FlagsDocked):
                ...
                if not self.jn.ship_state()['has_adv_dock_comp']:
                    ...
                    logging.warning('Unable to undock. Advanced Docking Computer not fitted.')
    ...
    def fsd_assist(self, scr_reg):
        ...
        logging.debug('self.jn.ship_state='+str(self.jn.ship_state()))
        ...
        while self.jn.ship_state()['target']:
            ...
            if self.jn.ship_state()['status'] == 'in_space' or self.jn.ship_state()['status'] == 'in_supercruise':
                ...
                if not position_ok:
                    ...
                    logging.warning("FSD assist aborted during positioning.")
                    break
    ...
    def sc_assist(self, scr_reg, do_docking=True):
        logging.debug("Entered sc_assist")
        ...
        if not self.have_destination(scr_reg):
            ...
            logging.debug("Quiting sc_assist - compass not found")
            return
    ...
    def set_log_file_disabled(self, enable=False):
        ...
        logging.getLogger().disabled = enable
    ...
    def engine_loop(self):
        while not self.terminate:
            ...
            if self.fsd_assist_enabled == True:
                logging.debug("Running fsd_assist")
                set_focus_elite_window()
                self.update_overlay()
                self.jump_cnt = 0
                self.refuel_cnt = 0
                ...
                try:
                    ...
                    logging.debug("Caught stop exception")
                except Exception as e:
                    print("Trapped generic:"+str(e))
                    traceback.print_exc()

                self.fsd_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('fsd_stop')
                self.update_overlay()
                ...
                if fin == False:
                    if self.ap_ckb: self.ap_ckb("sc_start")
                ...
                logging.debug("Running sc_assist")
                set_focus_elite_window()
                self.update_overlay()
                try:
                ...
                logging.debug("Completed sc_assist")
                self.sc_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('sc_stop')
                self.update_overlay()

            elif self.waypoint_assist_enabled == True:
                logging.debug("Running waypoint_assist")
                ...
                self.waypoint_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('waypoint_stop')
                self.update_overlay()

            elif self.robigo_assist_enabled == True:
                logging.debug("Running robigo_assist")
                set_focus_elite_window()
                self.update_overlay()
                try:
                    self.robigo_assist()
                ...
                self.robigo_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('robigo_stop')
                self.update_overlay()
                ...
                try:
                    ...
                    logging.debug("Stopping afk_combat")
                self.afk_combat_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('afk_stop')
                self.update_overlay()

            elif self.dss_assist_enabled == True:
                logging.debug("Running dss_assist")
                set_focus_elite_window()
                self.update_overlay()
                try:
                    self.dss_assist()
                except EDAP_Interrupt:
                    logging.debug("Stopping DSS Assist")
                self.dss_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('dss_stop')
                self.update_overlay()
                ...
                try:
                    ...
                    logging.debug("Stopping Single Waypoint Assist")
                self.single_waypoint_enabled = False
                if self.ap_ckb: self.ap_ckb('single_waypoint_stop')
                self.update_overlay()

            elif self.fc_assist_enabled == True:
                logging.debug("Running fc_assist")
                set_focus_elite_window()
                self.update_overlay()
                try:
                ...
                self.fc_assist_enabled = False
                if self.ap_ckb: self.ap_ckb('fc_stop') # This will need to be added to the GUI
                self.update_overlay()

            elif self.wing_mining_assist_enabled:
                logging.debug("Running wing_mining_assist")
                try:
                    ...
                    if self.wing_mining.state == "IDLE" or self.wing_mining.state == "DONE":
                        ...
                        if self.ap_ckb: self.ap_ckb('wing_mining_stop')
                except EDAP_Interrupt:
                    logging.debug("Caught stop exception for Wing Mining")
                    self.set_wing_mining_assist(False)
                    if self.ap_ckb: self.ap_ckb('wing_mining_stop')
                except Exception as e:
...
# Global variable to hold the autopilot instance
ed_ap = None
# Queue for events to be sent to the client
event_queue = queue.Queue()


class SingleWaypointRequest(BaseModel):
    system: str
    station: str

def server_callback(msg, body=None):
    """
    A callback function for the autopilot to send status updates.
    This will put events into a queue to be retrieved by the client.
    """
    event_queue.put({"type": msg, "payload": body})
    # Also log it for server-side debugging
    logging.info(f"Event queued: {msg} - {body}")

@app.on_event("startup")
async def startup_event():
    """
    This function will be called when the server starts up.
    It creates the EDAutopilot instance.
    """
    global ed_ap
    # We pass a simple logging callback for now.
    # The GUI will poll for status via an endpoint.
    ed_ap = EDAutopilot(cb=server_callback)
    logging.info("EDAutopilot server started.")

@app.get("/status")
async def get_status():
    """
    Returns the current status of the autopilot.
    """
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    return {
        "fsd_assist_enabled": ed_ap.fsd_assist_enabled,
        "sc_assist_enabled": ed_ap.sc_assist_enabled,
        "waypoint_assist_enabled": ed_ap.waypoint_assist_enabled,
        "robigo_assist_enabled": ed_ap.robigo_assist_enabled,
        "afk_combat_assist_enabled": ed_ap.afk_combat_assist_enabled,
        "dss_assist_enabled": ed_ap.dss_assist_enabled,
        "single_waypoint_enabled": ed_ap.single_waypoint_enabled,
        "fc_assist_enabled": ed_ap.fc_assist_enabled,
        "wing_mining_assist_enabled": ed_ap.wing_mining_assist_enabled,
        "ap_state": ed_ap.ap_state,
        "ship_state": ed_ap.jn.ship_state(),
    }
...
async def get_config():
    ...
    return {
        "config": ed_ap.config,
        "ship_configs": ed_ap.ship_configs,
        "pitch": ed_ap.pitchrate,
        "roll": ed_ap.rollrate,
        "yaw": ed_ap.yawrate,
...
async def update_config(request: ConfigUpdateRequest):
    ...
    ed_ap.config = request.config
    ed_ap.ship_configs = request.ship_configs

    # Also update individual ship parameters from the new config if they exist
    # This assumes the GUI sends the complete, updated config dictionaries
    ship_type = ed_ap.current_ship_type
    if ship_type and ship_type in ed_ap.ship_configs.get("Ship_Configs", {}):
        ship_cfg = ed_ap.ship_configs["Ship_Configs"][ship_type]
        ed_ap.pitchrate = ship_cfg.get('PitchRate', ed_ap.pitchrate)
        ed_ap.rollrate = ship_cfg.get('RollRate', ed_ap.rollrate)
        ed_ap.yawrate = ship_cfg.get('YawRate', ed_ap.yawrate)
        ed_ap.sunpitchuptime = ship_cfg.get('SunPitchUp+Time', ed_ap.sunpitchuptime)
...
@app.post("/waypoints/reset")
async def reset_waypoints():
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    ed_ap.waypoint.mark_all_waypoints_not_complete()
    return {"status": "Waypoints reset"}

class WaypointUpdateRequest(BaseModel):
    waypoints: dict

class WaypointLoadRequest(BaseModel):
    filename: str
    content: dict
...
async def get_waypoints():
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    return {"waypoints": ed_ap.waypoint.waypoints}

@app.post("/waypoints/load")
async def load_waypoints(request: WaypointLoadRequest):
    if not ed_ap:
        return {"error": "Autopilot not initialized"}

    # The client sends the content, so we load it.
    # The `load_waypoint_file` in EDWayPoint reads from a file, so we can't use it directly.
    # We will just set the waypoints and filename. Some validation might be good here.
    # For now, we trust the client sends a valid waypoint file content.
    ed_ap.waypoint.waypoints = request.content
    ed_ap.waypoint.filename = request.filename

    # We can add a log message on the server.
    logging.info(f"Loaded waypoints from {request.filename} via API.")

    return {"success": True, "message": "Waypoints loaded"}

@app.post("/waypoints/save")
async def save_waypoints(request: WaypointUpdateRequest):
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    ed_ap.waypoint.waypoints = request.waypoints
    ed_ap.waypoint.write_waypoints(data=ed_ap.waypoint.waypoints, filename=ed_ap.waypoint.filename)
    return {"status": "Waypoints saved"}

class WaypointMoveRequest(BaseModel):
    key: str
    direction: str

@app.post("/waypoints/move")
async def move_waypoint(request: WaypointMoveRequest):
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    ed_ap.waypoint.move_waypoint(request.key, request.direction)
    return {"status": "Waypoint moved"}

class WaypointAddRequest(BaseModel):
    waypoint: dict

@app.post("/waypoints/add")
async def add_waypoint(request: WaypointAddRequest):
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    # This assumes the waypoint key is part of the waypoint dict
    key = next(iter(request.waypoint))
    ed_ap.waypoint.waypoints[key] = request.waypoint[key]
    ed_ap.waypoint.write_waypoints(ed_ap.waypoint.waypoints, ed_ap.waypoint.filename)
    return {"status": "Waypoint added"}

class WaypointDeleteRequest(BaseModel):
    key: str

@app.post("/waypoints/delete")
async def delete_waypoint(request: WaypointDeleteRequest):
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    if request.key in ed_ap.waypoint.waypoints:
        del ed_ap.waypoint.waypoints[request.key]
        ed_ap.waypoint.write_waypoints(ed_ap.waypoint.waypoints, ed_ap.waypoint.filename)
        return {"status": "Waypoint deleted"}
...
async def get_events():
    """
    Retrieves all events from the queue.
    """
    events = []
...
@app.post("/start/fsd")
async def start_fsd_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_fsd_assist(True)
    return {"status": "FSD assist started"}

@app.post("/start/sc")
async def start_sc_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_sc_assist(True)
    return {"status": "SC assist started"}

@app.post("/start/waypoint")
async def start_waypoint_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_waypoint_assist(True)
    return {"status": "Waypoint assist started"}

@app.post("/start/robigo")
async def start_robigo_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_robigo_assist(True)
    return {"status": "Robigo assist started"}

@app.post("/start/dss")
async def start_dss_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_dss_assist(True)
    return {"status": "DSS assist started"}

@app.post("/start/single_waypoint")
async def start_single_waypoint_assist(request: SingleWaypointRequest):
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_single_waypoint_assist(request.system, request.station, True)
    return {"status": "Single Waypoint assist started"}

@app.post("/start/fc")
async def start_fc_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_fc_assist(True)
    return {"status": "FC assist started"}

@app.post("/start/wing_mining")
async def start_wing_mining_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_wing_mining_assist(True)
    return {"status": "Wing Mining assist started"}

@app.post("/start/afk_combat")
async def start_afk_combat_assist():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.set_afk_combat_assist(True)
    return {"status": "AFK Combat assist started"}


@app.post("/stop/{assist_type}")
async def stop_assist(assist_type: str):
    """
    Stops a specific assist mode.
    """
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    ...
    if assist_type == "fsd":
        ...
        return {"status": "FSD assist stopped"}
    elif assist_type == "sc":
        ed_ap.set_sc_assist(False)
        return {"status": "SC assist stopped"}
    elif assist_type == "waypoint":
        ed_ap.set_waypoint_assist(False)
        return {"status": "Waypoint assist stopped"}
    elif assist_type == "robigo":
        ed_ap.set_robigo_assist(False)
        return {"status": "Robigo assist stopped"}
    elif assist_type == "dss":
        ed_ap.set_dss_assist(False)
        return {"status": "DSS assist stopped"}
    elif assist_type == "single_waypoint":
        ed_ap.set_single_waypoint_assist("", "", False)
        return {"status": "Single Waypoint assist stopped"}
    elif assist_type == "fc":
        ed_ap.set_fc_assist(False)
        return {"status": "FC assist stopped"}
    elif assist_type == "wing_mining":
        ed_ap.set_wing_mining_assist(False)
        return {"status": "Wing Mining assist stopped"}
    elif assist_type == "afk_combat":
        ed_ap.set_afk_combat_assist(False)
        return {"status": "AFK Combat assist stopped"}
    else:
        return {"error": "Unknown assist type"}

@app.post("/stop_all")
async def stop_all_assists():
    """
    Stops all running assists.
    """
    if not ed_ap:
        return {"error": "Autopilot not initialized"}
    ...
    return {"status": "All assists stopped"}

@app.post("/calibrate/target")
async def calibrate_target():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.calibrate_target()
    return {"status": "Target calibration started"}

@app.post("/calibrate/compass")
async def calibrate_compass():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.calibrate_compass()
    return {"status": "Compass calibration started"}

@app.post("/test/pitch")
async def test_pitch():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.ship_tst_pitch()
    return {"status": "Pitch test started"}

@app.post("/test/roll")
async def test_roll():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.ship_tst_roll()
    return {"status": "Roll test started"}

@app.post("/test/yaw")
async def test_yaw():
    if not ed_ap: return {"error": "Autopilot not initialized"}
    ed_ap.ship_tst_yaw()
    return {"status": "Yaw test started"}
